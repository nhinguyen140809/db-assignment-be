...
AS
    BEGIN
        ...
        BEGIN TRY
            ...
            -- Check if a menu item with the same name already exists in this restaurant
            IF EXISTS
                (
                    SELECT
                        1
                ...
                BEGIN
                    RAISERROR('An available/unavailable menu item with the same name already exists in this restaurant.', 16, 1);
                    ROLLBACK TRANSACTION;
                    RETURN;
                END;
            ...
            SELECT
                @NextNumber = ISNULL(MAX(CAST(SUBSTRING([food_id], 4, LEN([food_id])) AS BIGINT)), 0) + 1
            FROM
                [menu_item] WITH (UPDLOCK, HOLDLOCK)
            WHERE
                [restaurant_id] = @RestaurantID;
            ...
            COMMIT TRANSACTION;
            SELECT
                @NewMenuItemID AS NewMenuItemID;
